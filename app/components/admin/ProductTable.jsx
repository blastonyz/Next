...
const ProductTable = async () => {
    const items = await fetch(`${process.env.API_URL}/api/productos/todos`,{cache: "no-store"}).then(r => r.json())
                    ...
                    <tbody>
                        {
                                ...
                                <tr key={item.id}>
                                    ...
                                    <td className="p-2">
                                        <Boton className="bg-green-400">
                                            <Link href={`${process.env.API_URL}/admin/edit/${item.id}`}>editar</Link>
                                        </Boton>
                                    </td>
                                </tr>
                            ))
                            ...
                            <Boton className="bg-orange-500">
                             <Link href={`${process.env.API_URL}/admin/create`}>Crear +</Link>
                            </Boton>
                            <Boton className="bg-orange-500">
                            <Link href={`${process.env.API_URL}/admin/orders`}>Ventas</Link>
                            </Boton>               
                            </div>
        </div>
    )
